...
type Provider interface {
	WithContext(ctx common.Context) Provider
...
func (p *provider) Context() common.Context {
	return p.ctx
}

func (p *provider) WithContext(ctx common.Context) Provider {
	if ctx == nil {
		panic("nil context")
	}
	p2 := new(provider)
