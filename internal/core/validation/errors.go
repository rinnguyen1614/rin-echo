...
func NewValidationError(id, message string) *ValidationError {
	return &ValidationError{
		...
		RinError:    core.NewRinError(id, message),
	}
}
...
func NewValidationErrorWithFields(err error, fieldErrors []FieldError) *ValidationError {
	return &ValidationError{
		...
		RinError:    core.NewRinErrorWithInner(err, "validation_error", "You have some errors for validation."),
	}
}
...
func NewFieldError(err validator.FieldError) FieldError {
	...
	f := fieldError{
		FieldError: err,
		RinError:   core.NewRinErrorWithInner(err, err.Field()+"_invalid", message),
	}
